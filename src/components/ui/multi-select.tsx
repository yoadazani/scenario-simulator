...
type MultiSelectContextType = {
    ...
    items: Map<string | number, ReactNode>
...
export function MultiSelect({
    ...
    children: ReactNode
    values?: string[]
    initialItems?: {value: string | number, label: ReactNode | string}[]
    ...
    const [items, setItems] = useState<Map<string | number, ReactNode>>(
        new Map(initialItems?.map(item => [item.value, item.label]) ?? [])
    );
...
export function MultiSelectTrigger({
    ...
    return (
        <PopoverTrigger asChild>
            ...
            >
                {children}
                <ChevronsUpDownIcon className="size-4 shrink-0 opacity-75"/>
            </Button>
        </PopoverTrigger>
    )
}
